...
class Cube(object):
    ...
    def __init__(self, ident: tuple, n: int, scale: int) -> None:
        ...
        self.face_colors = [0, 1, 2, 3, 4, 5]  # Default color indices

    def set_colors(self, color_dict):
        face_order = ['U', 'R', 'F', 'D', 'L', 'B']
        for i, face in enumerate(face_order):
            color_index = color_dict[f'{face}5']  # Center piece of each face
            self.face_colors[i] = color_index
    ...
    def draw(self, surf, animate, angle, axis, slc, dr):
        ...
        for i in range(len(surf)):
            color = self.color_map[self.colors[self.face_colors[i]]]
            GL.glColor3fv(color)
            for j in surf[i]:
        ...
        GL.glColor3fv((0, 0, 0))
        for edge in self.edges: #changed from Cube to self
            for vertex in edge:
