...
print("Click on the video feed to select points in the order of top-left, top-right, bottom-left, bottom-right for each grid (3 sets of 4 points). Upside Grid First then left then right.")
...
def get_next_set_number():
    ...
    while True:
        ...
        files_with_prefix_or_substring = any(fn.startswith(prefix) for fn in os.listdir('../training'))
        files_two = any(substring in fn for fn in os.listdir('../training'))
...
while True:
    ...
    if len(points) == 12:
        ...
        grid_labels = ['U', 'L', 'R']
        cv2.imwrite('full_test_image.png',frame)
            ...
            for i in range(3):
                for j in range(3):
                    ...
                    if top_left_x >= 0 and top_left_y >= 0 and bottom_right_x <= img.shape[1] and bottom_right_y <= \
                        ...
                        print(f"{grid_label}{3 * i + j + 1}: ({top_left_x}, {top_left_y}, 32, 32),")
