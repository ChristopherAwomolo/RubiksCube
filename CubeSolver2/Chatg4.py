...
print("Click on the video feed to select points in the order of top-left, top-right, bottom-left, bottom-right for each grid (3 sets of 4 points).")
...
while True:
    ...
    if len(points) == 12:
        ...
        for grid_index in range(3):  # 3 grids
            ...
            bottom_third2 = (int(grid_points[2][0] + 2 * (grid_points[3][0] - grid_points[2][0]) / 3), int(grid_points[2][1] + 2 * (grid_points[3][1] - grid_points[2][1]) / 3))
            '''
            ...
            left_third1 = (int(grid_points[0][0] + (grid_points[2][0] - grid_points[0][0]) / 3), int(grid_points[0][1] + (grid_points[2][1] - grid_points[0][1]) / 3))
            left_third2 = (int(grid_points[0][0] + 2 * (grid_points[2][0] - grid_points[0][0]) / 3), int(grid_points[0][1] + 2 * (grid_points[2][1] - grid_points[0][1]) / 3))
            right_third1 = (int(grid_points[1][0] + (grid_points[3][0] - grid_points[1][0]) / 3), int(grid_points[1][1] + (grid_points[3][1] - grid_points[1][1]) / 3))
            right_third2 = (int(grid_points[1][0] + 2 * (grid_points[3][0] - grid_points[1][0]) / 3), int(grid_points[1][1] + 2 * (grid_points[3][1] - grid_points[1][1]) / 3))
            ...
            for i in range(3):
                for j in range(3):
                    ...
                    top_left = interpolate(interpolate(grid_points[0], grid_points[2], i / 3), interpolate(grid_points[1], grid_points[3], i / 3), j / 3)
                    top_right = interpolate(interpolate(grid_points[0], grid_points[2], i / 3), interpolate(grid_points[1], grid_points[3], i / 3), (j + 1) / 3)
                    bottom_left = interpolate(interpolate(grid_points[0], grid_points[2], (i + 1) / 3), interpolate(grid_points[1], grid_points[3], (i + 1) / 3), j / 3)
                    bottom_right = interpolate(interpolate(grid_points[0], grid_points[2], (i + 1) / 3), interpolate(grid_points[1], grid_points[3], (i + 1) / 3), (j + 1) / 3)
